...
def search(start, board, costs, goals):    #ucs algorithm
    ...
    visited_nodes = {start: Node(position=start, previous=None, cost=0)}
    ...
    while priority_queue:       #algo start
        ...
        for next_position in board.getLegalMovesPositions(current_x_int, current_y_int):
            if next_position not in visited_nodes:
                visited_nodes[next_position] = Node(position=next_position, previous=current_position, cost=-1)
                if next_position not in threatened and next_position not in blocked:
                    ...
                    frontier_node = (visited_nodes[current_position].cost() + action_cost, next_position)
