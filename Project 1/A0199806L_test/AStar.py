...
##  =============================================================================================   ##
##  3. Functions:                                #T_FNC                                             ##
##      - search(): does the AStar                                                                  ##
##      - run_AStar(): parse the file and initialise the state and environment                      ##
##  =============================================================================================   ##
##  4. Main     #T_MAIN                                                                             ##
##                                                                                                  ##
######################################################################################################
...
def search(start, board, costs, goals):    #AStar algorithm
    '''The algorithm for AStar.'''
    #----------------- heuristic --------------------
    def h(state: str) -> int:
        '''Using Chebychev distance as a heuristic.'''
        state_x = INT(state[0])
        state_y = int(state[1:])
        min_chebychev = -1
        for goal in goals:
            goal_x = INT(goal[0])
            goal_y = int(goal[1:])
            x_distance = abs(state_x - goal_x)
            y_distance = abs(state_y - goal_y)
            current_chebychev = max(x_distance, y_distance)
            if min_chebychev < 0 or current_chebychev < min_chebychev:
                min_chebychev = current_chebychev
        return min_chebychev
    #--------------- heuristic end ----------------
    ...
    visited_nodes = {start: Node(start, None, 0)}
    ...
    while priority_queue:                                            # second iteration onwards
        current_position = heapq.heappop(priority_queue)[1]
        if current_position in goals:   # goal test at pop()
            return visited_nodes[current_position].getPath(visited_nodes), len(visited_nodes), visited_nodes[current_position].cost()
        ...
        for next_position in board.getLegalMovesPositions(current_x_int, current_y_int):
            if next_position not in visited_nodes:
                visited_nodes[next_position] = Node(next_position, current_position, -1)
                if next_position not in threatened and next_position not in blocked:
                    ...
                    frontier_node = (visited_nodes[current_position].cost() + action_cost + h(next_position), next_position)
                    visited_nodes[next_position].setCost(visited_nodes[current_position].cost() + action_cost)
                    heapq.heappush(priority_queue, frontier_node)
                    #cannot goal test here, might not be optimal
            else: #check relax
                action_cost = costs[next_position] if next_position in costs else 1
                if visited_nodes[next_position].relax(visited_nodes[current_position].cost() + action_cost, current_position): #can relax so push back to heap
                    action_cost = costs[next_position] if next_position in costs else 1
                    frontier_node = (visited_nodes[current_position].cost() + action_cost + h(next_position), next_position)
                    heapq.heappush(priority_queue, frontier_node)
...
## ******************** !!defining main ************************
def run_AStar():                  #main definition: parsing of file and initialising environment       ####T_ASTAR
