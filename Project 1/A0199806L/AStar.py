...
class Node:
    def __init__(self, position, previous=None, cost=-1, h=-1):
...
def search(start, board, costs, goals):    #AStar algorithm
    ...
    blocked = board.blocked()
    priority_queue = [Node(position=start, previous=None, cost=0, h=h(start))]
    visited_nodes = {}
    ...
    while priority_queue:                                            # second iteration onwards
        ...
        if current_position not in visited_nodes: #add to visited on pop
            ...
            for next_position in board.getLegalMovesPositions(current_x_int, current_y_int):
                if next_position not in threatened and next_position not in blocked:
                    ...
                    frontier_node = Node(position=next_position, previous=current_position, cost=current_node.cost() + action_cost, h=h(next_position))
