...
class Piece:                      # base class                                  ####T_PIECE
    ...
    def __eq__(self, other_piece):
        return self._position == other_piece.position()
...
class Board:
    def __init__(self, rows, columns, pieces, obstacles, visited):
        ...
        self._pieces: set = pieces #list of strings representing position of each obstacle ## -> List[str]
        self._blocked = set()
        self._hash_value = frozenset(self._pieces)
        ...
        if self in visited:
            self._heuristic = visited[self]
        else:
            self._heuristic = self.calculateHeuristic()
            visited[self] = self._heuristic
    ...
    def __eq__(self, other_board):
        return self._pieces == other_board.pieces()
    ...
    def getSuccessor(self, i, visited):
        new_pieces = self._pieces.copy()
        new_pieces.remove(i)
        return Board(self._ROWS, self._COLUMNS, new_pieces, self._obstacles, visited)
...
##========================== FUNCTIONS START ===========================   
def search(rows, columns, pieces, obstacles, k):    #Hill climbing algorithm
    '''The algorithm for local search.'''
    # while True:
    for iteration in range(60):
        ### ====================================== 1.RANDOM RESTART =================================
        board = Board(rows, columns, random.sample(pieces, k=random.randint(k,len(pieces))), obstacles, {})
        visited = {board: board.heuristic()}
        if board.goalTest():
            return board.state()
        while board:    ###while current restart climbs
            ### --------- 3 list of successors split into <h, ==h, >h --------------
            lower_successors = []
            equal_successors = []
            higher_successors = []
            ### ------------------------------------------------------------------
            ...
            ### ================================= 2. CHECKING SUCCESSORS ===================================
            if number_of_pieces > k: #check if already reach terminal state
                for i in board.pieces():   ### for every piece, want to remove 1 by 1 to generate all successors
                    if random.random() < goal_current_ratio:    ### for current piece, randomly decide whether to remove this piece and check h
                        successor = board.getSuccessor(i, visited)           ### decided to check this piece, so remove this piece from the board
                        successor_h = successor.heuristic()         ### calculate h for the board without this piece
                        ### ------------------------ put into the respective lists based on its h compared with the current h ---------------
                        if(successor_h < current_h):
                            if successor.goalTest():
                                return successor.state()
                            lower_successors.append(successor)
                        elif(successor_h == current_h):
                            if successor.goalTest():
                                return successor.state()
                            equal_successors.append(successor)
                        else:
                            higher_successors.append(successor)

            board = None
            #### ===================== 3. SELECTING SUCCESSORS ==========================
            if lower_successors:        ### if have strictly less than h, then take the minimum first
                heapq.heapify(lower_successors)
                board = heapq.heappop(lower_successors)
            elif equal_successors:          ### if dont have then take a successor with equal h
                board = equal_successors[0]
            elif higher_successors and random.random < goal_current_ratio:      ### if dont have less than or equal h, then randomly decide whether to take a higher h successor
                board = higher_successors[0]

            # if lower_successors:
            #     heapq.heapify(lower_successors)
            #     min_successors = []
            #     min_board = heapq.heappop(lower_successors)
            #     min_successors.append(min_board)
            #     min_h = min_board.heuristic()
            #     if lower_successors:
            #         successor = heapq.heappop(lower_successors)
            #         successor_h = successor.heuristic()
            #         while successor_h == min_h and lower_successors:
            #             min_successors.append(successor)
            #             successor = heapq.heappop(lower_successors)
            #             successor_h = successor.heuristic()
            #     board = random.choice(min_successors)
            # elif equal_successors:
            #     board = random.choice(equal_successors)
