...
def search(rows, columns, pieces, obstacles, k):    #bfs algorithm
    ...
    if initial_h == 0:
        return board.state()
    priority_queue = [(initial_h, board.pieces())]
    counter = 50
    while priority_queue and counter:
        current_h, remaining_pieces = heapq.heappop(priority_queue)
        if len(remaining_pieces) > k:
            for i in remaining_pieces:
                successor_remaining_pieces = board.getSuccessor(i, remaining_pieces)
                ...
                heapq.heappush(priority_queue, (successor_h, successor_remaining_pieces))
        counter -= 1
    return None
