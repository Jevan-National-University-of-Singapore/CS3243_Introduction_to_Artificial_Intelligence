...
def search(rows, columns, pieces, obstacles, k):    #bfs algorithm
    ...
    if board.goalTest(board.pieces()):
        return board.state()
    priority_queue = [(board.heuristic(), board.pieces())]
    counter = 2000
    while priority_queue and counter:
        ...
        if len(remaining_pieces) > k:
            for i in remaining_pieces:
                ...
                if board.goalTest(successor_remaining_pieces):
                    return board.state(successor_remaining_pieces)
                heapq.heappush(priority_queue, (board.heuristic(successor_remaining_pieces), successor_remaining_pieces))
        counter -= 1
