...
class Board:
    ...
    def calculateHeuristic(self) -> int:
        ...
        for piece in self._pieces:
            for check_threatened in self._pieces:
                threatened = set()
                if check_threatened.position() in piece.getThreats(threatened, self):
    ...
    def getSuccessor(self, i):
        ...
        # return Board(self._ROWS, self._COLUMNS, random.choices(self._pieces, k=len(self._pieces)-1), self._obstacles)
    ...
    while True:
        ...
        while beams:
            ...
            if not all_successors:
                break
            beams = []
            while all_successors and len(beams) <=k:
