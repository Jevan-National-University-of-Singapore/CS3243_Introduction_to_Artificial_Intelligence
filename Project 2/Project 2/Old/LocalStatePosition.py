...
class Piece:                      # base class                                  ####T_PIECE
    ...
    def threats(self) -> set:
        return self._threats
    
    @abstractmethod
    def getThreats(self, board: "Board"):
        pass
...
class Knight(Piece):                        
    ...
    def getThreats(self, board: "Board") -> set:
...
class Board:
    def __init__(self, rows, columns, pieces, obstacles):
        ...
        self._pieces = {} #list of strings representing position of each obstacle ## -> List[str]
        self._blocked = set()
        ...
        for piece in pieces:
            self._pieces[piece] = Piece.create(pieces[piece], piece, self)
    ...
    def heuristic(self, remaining_pieces = None):
        ...
        if remaining_pieces is None:
            for piece in self._pieces:
                ...
                #     print(check_threatened)
                #     print("-------")
                    check_threatened = self._pieces[check_threatened]
    ...
    # def goalTest(self, removed_pieces = set()) -> bool:
    #     return self.heuristic(removed_pieces) == 0
    

    def getSuccessor(self, piece, removed_pieces, remaining_pieces):
        removed_pieces_new = removed_pieces.copy()
        remaining_pieces_new = remaining_pieces.copy()
        removed_pieces_new.add(piece)
        remaining_pieces_new.remove(piece)
        return removed_pieces_new, remaining_pieces_new
...
def search(rows, columns, pieces, obstacles, k):    #bfs algorithm
    ...
    initial_h = board.heuristic()
    if initial_h == 0:
        return board.state()
    priority_queue = [(initial_h, set(), board.pieces())]
    while priority_queue:
        current_h, removed_pieces, remaining_pieces = heapq.heappop(priority_queue)
        if len(remaining_pieces) > k:
            for i in remaining_pieces:
                successor_removed_pieces, successor_remaining_pieces = board.getSuccessor(i, removed_pieces, remaining_pieces)
                successor_h = board.heuristic(successor_remaining_pieces)
                # print(successor_h)
                if(successor_h < current_h):
                    if successor_h == 0:
                        return board.state(successor_remaining_pieces)
                    heapq.heappush(priority_queue, (successor_h, successor_removed_pieces, successor_remaining_pieces))
...
def run_local():                                                                        ####T_BFS
    ...
    while piece:
        ...
        pieces[piece_position] = piece_type     
        piece = next(testfile)
