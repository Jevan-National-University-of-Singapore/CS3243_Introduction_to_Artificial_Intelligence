...
class Piece:                      # base class                                  ####T_PIECE
    ...
    def __lt__(self, piece):
        return (self.x(), self.y()) < (piece.x(), piece.y())
...
class Board:
    def __init__(self, rows, columns, pieces, obstacles, visited = {}):
        ...
        self._check: tuple = ((i.x(), i.y()) for i in pieces)
        self._blocked = set()
    ...
    def __eq__(self, board: "Board") -> bool:
        return self._check == board.check()
...
def search(rows, columns, pieces, obstacles, k):    #bfs algorithm
    ...
    # for iterations in range(100):
        ...
        board = Board(rows, columns, random.sample(pieces, k=random.randint(k,len(pieces))), obstacles, visited)
        ...
        while board:
            ...
            if len(board.pieces()) > k:
                for i in range(len(board.pieces())):
                    if random.random() < k/len(board.pieces()):              ###>0.01: 197.473          ##>0.1: 128.39            #<k/len(pieces): 59.34s  ###queen: 159.16 max, total 1856
                        successor = board.getSuccessor(i, visited)
                        # print(len(visited))
                        ...
                        elif random.random() < k/len(board.pieces()):       ###< k/len:49.86
...
t1 = time()
for i in range(30):
    print(f"{run_local()}       {time()-t1}")                                          #####T_MAIN
    t1 = time()
print(time() - t0)