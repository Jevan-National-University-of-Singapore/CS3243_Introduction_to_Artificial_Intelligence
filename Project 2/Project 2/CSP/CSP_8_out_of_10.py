...
class Board:
    ...
    def addPiece(self, added_piece, added_threats) -> bool:
        # print("")
        # for i in self._enemy_pieces:
        #     print(f"{i.position()}  {i.type()}")
        # print(f"adding: {added_piece.position()}    {added_piece.type()}")
        self._enemy_pieces.add(added_piece)
        self._invalid_positions = set.union(self._invalid_positions, added_threats)
        self._invalid_positions.add(added_piece.position())
...
def search(rows, columns, pieces, obstacles):    #Hill climbing algorithm
    ...
    start = Board(rows, columns, set(), obstacles)
    stack = [(pieces, start)]         #frontier
    piece_threats = {"King":{}, "Queen":{}, "Bishop":{}, "Rook":{}, "Knight":{}}
    visited = {start}
    counter = 100000
    while stack and counter:                                          #algo start
        ...
        for x in range(columns):
            for y in range(rows):
                ...
                if position not in invalid_positions:
                    ...
                    if position in piece_threats[assignment]:
                        current_piece_threat = piece_threats[assignment][position]
                    else:
                        current_piece_threat = new_piece.getThreats(current_state, obstacles)
                        piece_threats[assignment][position] = current_piece_threat
                    violated = False
                    ...
                    if not violated:
                        ...
                        successor.addPiece(new_piece, piece_threats[assignment][position])
