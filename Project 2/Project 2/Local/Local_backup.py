    ...
    while True:
        ...
        while board:    ###while current restart climbs
            ...
            if number_of_pieces > k: #check if already reach terminal state
                for i in range(number_of_pieces):   ### for every piece, want to remove 1 by 1 to generate all successors
                    if random.random() < goal_current_ratio:    ### for current piece, randomly decide whether to remove this piece and check h
                        successor = board.getSuccessor(i)           ### decided to check this piece, so remove this piece from the board
