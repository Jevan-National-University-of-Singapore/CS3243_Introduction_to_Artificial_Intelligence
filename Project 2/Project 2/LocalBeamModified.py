...
def search(rows, columns, pieces, obstacles, k):    #bfs algorithm
    ...
    # while True:
    for iterations in range(100000000000000):
        print(f"i: {iterations}")
        beams = []
        num_of_beams = max(round(math.sqrt(len(pieces))), 1)
        while len(beams) < num_of_beams:
        ...
        while beams:
            all_successors = []
            for state in beams:
                current_h = state.heuristic()
                if len(state.pieces()) > k:
                    for i in range(len(state.pieces())):
                        successor = state.getSuccessor(i)
            ...
            beams = []
            num_of_beams = max(round(math.sqrt(len(all_successors))), 1)
            if all_successors:
                heapq.heapify(all_successors)
                while len(beams) < num_of_beams and all_successors:
                    beams.append(heapq.heappop(all_successors))
            print(num_of_beams)
                    # min_successors = []
                    # min_board = heapq.heappop(all_successors)
                    # min_successors.append(min_board)
                    # min_h = min_board.heuristic()
                    # if all_successors:
                    #     successor = heapq.heappop(all_successors)
                    #     successor_h = successor.heuristic()
                    #     while successor_h == min_h and all_successors:
