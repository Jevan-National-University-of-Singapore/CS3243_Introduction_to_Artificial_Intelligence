...
class Knight(Piece):                        
    def __init__(self, x, y, team):
        super().__init__(x, y, "Knight", team, 2)
...
class King(Piece):
    def __init__(self, x, y, team):
        self._team = team
        super().__init__(x, y, "King", team, 10) ## high enough heuristic such that without this piece, impossible to get best_value
...
class Bishop(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Bishop", team, 2) # heuristic = 4.16 average moves for each position on the board
...
class Queen(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Queen", team, 4)#12.16)
...
class Rook(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Rook", team, 3)
...
class Board:
    ...
    def getAllValidSuccessors(white_pieces, black_pieces, white_turn, heuristic):
        if white_turn:
            ...
            for position, piece in white_pieces.items():
                if piece.type() != "Pawn":
                    bai_se_de_pieces[piece.type()] = piece
                elif piece.type() not in bai_se_de_pieces:
                    bai_se_de_pieces[piece.type()] = [piece]
                else:
                    bai_se_de_pieces[piece.type()].append(piece)
            while bai_se_de_pieces:
                if "Queen" in bai_se_de_pieces:
                    piece = bai_se_de_pieces["Queen"]
                elif "Rook" in bai_se_de_pieces:
                    piece = bai_se_de_pieces["Rook"]
                elif "Bishop" in bai_se_de_pieces:
                    piece = bai_se_de_pieces["Bishop"]
                elif "Knight" in bai_se_de_pieces:
                    piece = bai_se_de_pieces["Knight"]
                elif "Pawn" in bai_se_de_pieces:
                    piece = bai_se_de_pieces["Pawn"].pop()
                elif "King" in bai_se_de_pieces:
                    piece = bai_se_de_pieces["King"]
                ...
                while current_move:
                    ...
                    new_white_pieces = Board.makeMove(white_pieces, (piece.x(), piece.y()), current_move)
                    ...
                    current_move = next(all_moves_of_piece)
                if piece.type() == "Pawn":
                    if not bai_se_de_pieces["Pawn"]:
                        bai_se_de_pieces.pop("Pawn")
                else:
                    bai_se_de_pieces.pop(piece.type())
        else:
            hei_se_de_pieces = {}
            piece = None
            for position, piece in black_pieces.items():
                if piece.type() != "Pawn":
                    hei_se_de_pieces[piece.type()] = piece
                elif piece.type() not in hei_se_de_pieces:
                    hei_se_de_pieces[piece.type()] = [piece]
                else:
                    hei_se_de_pieces[piece.type()].append(piece)
            while hei_se_de_pieces:
                if "Queen" in hei_se_de_pieces:
                    piece = hei_se_de_pieces["Queen"]
                elif "Rook" in hei_se_de_pieces:
                    piece = hei_se_de_pieces["Rook"]
                elif "Bishop" in hei_se_de_pieces:
                    piece = hei_se_de_pieces["Bishop"]
                elif "Knight" in hei_se_de_pieces:
                    piece = hei_se_de_pieces["Knight"]
                elif "Pawn" in hei_se_de_pieces:
                    piece = hei_se_de_pieces["Pawn"].pop()
                elif "King" in hei_se_de_pieces:
                    piece = hei_se_de_pieces["King"]
                ...
                while current_move:
                    ...
                    new_black_pieces = Board.makeMove(black_pieces, (piece.x(), piece.y()), current_move)
                    # yield new_white_pieces, new_black_pieces, Board.heuristic(new_black_pieces, new_white_pieces)
                    yield new_white_pieces, new_black_pieces, new_heuristic
                    current_move = next(all_moves_of_piece)
                if piece.type() == "Pawn":
                    if not hei_se_de_pieces["Pawn"]:
                        hei_se_de_pieces.pop("Pawn")
                else:
                   hei_se_de_pieces.pop(piece.type())
        yield None
    ...
    def heuristic(my_pieces, enemy_pieces):
        ...
        #         if (position) in Board.getAllThreats(my_pieces, enemy_pieces):
        #             points -= 15
        # for position, enemy_piece in enemy_pieces.items():
        #     if enemy_piece.type() == "King":
        #         if (position) in Board.getAllThreats(enemy_pieces, my_pieces):
        #             points += 15
        # return 0
        # num_my_pieces = len(my_pieces)
        # num_enemy_pieces = len(enemy_pieces)
        # stage = min(num_enemy_pieces, num_my_pieces)/10
        # # if stage > 0.8:
        # #     all_enemy_threats = Board.getAllThreats(my_pieces, enemy_pieces)
        # #     all_friendly_threats = Board.getAllThreats(enemy_pieces, my_pieces)
        # #     return 0.25*(1-stage)*(num_my_pieces - num_enemy_pieces) + 0.25*stage*(len(all_friendly_threats)-len(all_enemy_threats))
        # return 0.5*(1-stage)*(num_my_pieces - num_enemy_pieces)
        return 0
...
def ab(node, depth, is_max, alpha, beta):
    ...
    if depth == 3:
        # if is_max:
        # for position, my_piece in white_pieces.items():
        #     if len(white_pieces) > 3:
        #         if my_piece.type() == "King":
        #             all_enemy_threats = Board.getAllThreats(white_pieces, black_pieces)
        #             if (position) in all_enemy_threats:
        #                 return move, heuristic-my_piece.heuristic
            # for position, enemy_piece in black_pieces.items():
            #     if enemy_piece.type() == "King":
            #         if (position) in all_friendly_threats:
            #             return move, heuristic + 500
        # else:
        #     for position, my_piece in black_pieces.items():
        #         if my_piece.type() == "King":
        #             all_enemy_threats = Board.getAllThreats(black_pieces, white_pieces) 
        #             if (position) in all_enemy_threats:
        #                 return move, heuristic+8
        #     # for position, enemy_piece in white_pieces.items():
        #     #     if enemy_piece.type() == "King":
        #     #         if (position) in all_friendly_threats:
        #     #             return move, heuristic + 10
    ...
    if is_max:
        ...
        while successor:
            ...
            if beta <= alpha:
                break
            successor=next(all_valid_successors)
        return best_move, highest_value if highest_value != -10000000 else heuristic
    else:
        ...
        while successor:
            ...
            if beta <= alpha:
                break
            successor = next(all_valid_successors)
        return best_move, lowest_value if lowest_value != 10000000 else heuristic
...
def studentAgent(gameboard):
    ...
    initial_heuristic = 0
    for piece in pieces:
        initial_heuristic += pieces[piece].heuristic
    for piece in enemy_pieces:
        initial_heuristic -= enemy_pieces[piece].heuristic
         
    all_valid_successors = Board.getAllInitialValidSuccessors(pieces, enemy_pieces, initial_heuristic)
    successor = next(all_valid_successors)
