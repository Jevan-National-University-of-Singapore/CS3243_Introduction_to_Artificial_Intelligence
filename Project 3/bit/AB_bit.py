...
class Board:
    def __init__(self, my_pieces, enemy_pieces):
        ...
        # self.white_pieces_bb = self.white_rook_bb | self.white_king_bb | self.white_bishop_bb | self.white_pawn_bb | self.white_knight_bb | self.white_queen_bb
        # self.black_pieces_bb = self.black_rook_bb | self.black_king_bb | self.black_bishop_bb | self.black_pawn_bb | self.black_knight_bb | self.black_queen_bb
    ...
    def getWhiteRookThreats(self):
        ...
        # north = np.uint64(north_attack) if block == 0 else np.uint64(north_attack ) ^ np.uint64(NORTH_ATTACK_BY_INDEX[bitscan_forward(block)])
        ...
        # west = np.uint64(west_attack) if block == 0 else np.uint64(west_attack) ^ np.uint64(WEST_ATTACK_BY_INDEX[bitscan_forward(block)])
        ...
        # south = np.uint64(south_attack) if block == 0 else np.uint64(south_attack) ^ np.uint64(SOUTH_ATTACK_BY_INDEX[bitscan_reverse(block)])
...
def getMovesFromBB(current_pos, bitboard):
    moves = set()
    while bitboard:
        index = bitscan_forward(bitboard)
        moves.add((current_pos, index))
