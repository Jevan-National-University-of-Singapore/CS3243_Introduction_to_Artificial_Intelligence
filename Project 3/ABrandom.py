...
class Knight(Piece):                        
    def __init__(self, x, y, team):
        super().__init__(x, y, "Knight", team, 1.78)
...
class King(Piece):
    def __init__(self, x, y, team):
        self._team = team
        super().__init__(x, y, "King", team, 1000) ## high enough heuristic such that without this piece, impossible to get best_value
...
class Bishop(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Bishop", team, 1.78) # heuristic = 4.16 average moves for each position on the board
...
class Queen(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Queen", team, 4.125)#12.16)
...
class Rook(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Rook", team, 2.56)
...
class Board:
        ...
        if white_turn:
            for position, piece in white_pieces.items():
                ...
                while current_move:
                    ...
                    yield new_white_pieces, new_black_pieces, new_heuristic + Board.heuristic(new_white_pieces, new_black_pieces)
                    current_move = next(all_moves_of_piece)
        else:
            for position, piece in black_pieces.items():
                ...
                while current_move:
                    ...
                    yield new_white_pieces, new_black_pieces, new_heuristic + Board.heuristic(new_black_pieces, new_white_pieces)
                    current_move = next(all_moves_of_piece)
        yield None
    ...
    def getAllInitialValidSuccessors(white_pieces, black_pieces, heuristic):
        for position, piece in white_pieces.items():
            ...
            while current_move:
                ...
                yield new_white_pieces, new_black_pieces, new_heuristic + Board.heuristic(new_white_pieces, new_black_pieces), ((piece.x(), piece.y()),current_move)
                current_move = next(all_moves_of_piece)
        yield None
    ...
    def heuristic(my_pieces, enemy_pieces):
        ...
        #             points += 500
        return points + len(all_friendly_threats) - len(all_enemy_threats)
...
def ab(node, depth, is_max, alpha, beta):
    ...
    if depth == 2:
        all_enemy_threats = Board.getAllThreats(white_pieces, black_pieces) if is_max else Board.getAllThreats(black_pieces, white_pieces) 
        all_friendly_threats = Board.getAllThreats(black_pieces, white_pieces)  if is_max else Board.getAllThreats(white_pieces, black_pieces)
        if is_max:
            for position, my_piece in white_pieces.items():
                if my_piece.type() == "King":
                    if (position) in all_enemy_threats:
                        return move, heuristic-200
    ...
    if is_max:
        ...
        while successor:
            ...
            if value > highest_value:
                ...
                highest.append(new_move)
            alpha = max(alpha, highest_value)
            if beta <= alpha:
                break
            successor=next(all_valid_successors)
            if highest:
                best_move = random.choice(highest)
        ...
        while successor:
            ...
            successor = next(all_valid_successors)
            if lowest:
                best_move = random.choice(lowest)
