...
class Knight(Piece):                        
    def __init__(self, x, y, team):
        super().__init__(x, y, "Knight", team, 8)

    def getMoves(self, my_pieces, enemy_pieces):
        return self.getThreats(set(), my_pieces, enemy_pieces)

    def getThreats(self, all_threats: set, my_team, enemy_team) -> set:
        return KNIGHT_THREATS[(self._x, self._y)]
...
class King(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "King", team, 400) ## high enough heuristic such that without this piece, impossible to get best_value
    
    def getMoves(self, my_pieces, enemy_pieces):
        return self.getThreats(set(), my_pieces, enemy_pieces)

    def getThreats(self, all_threats: set, my_team, enemy_team) -> set:
        return KING_THREATS[(self._x, self._y)]
...
class Bishop(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Bishop", team, 4) # heuristic = 4.16 average moves for each position on the board
...
class Queen(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Queen", team, 12)#12.16)
...
class Rook(Piece):
    def __init__(self, x, y, team):
        super().__init__(x, y, "Rook", team, 8)
...
class Pawn(Piece):
    def __init__(self, x, y, team):
        ...
        super().__init__(x, y, "Pawn", team, 2)

    def getThreats(self, all_threats: set, my_team, enemy_team) -> set:
        if self._y == self._y_limit:
            return all_threats
        if self._x != 0:
            all_threats.add((self._x - 1, self._y + self._y_movement))
        if self._x != 4:
            all_threats.add((self._x + 1, self._y + self._y_movement))
        return all_threats
    
    def getMoves(self, white_team, black_team):
        moves = set()
        my_team, enemy_team = (white_team, black_team) if self._team == "White" else (black_team, white_team)

        if self._y != self._y_limit:
            move_position = (self._x, self._y+self._y_movement)
            if move_position not in enemy_team and move_position not in my_team:
                moves.add(move_position)
            if self._x != 0:
    ...
    def getAllValidSuccessors(white_pieces, black_pieces, white_turn, heuristic):
        if white_turn:
            for position, piece in white_pieces.items():
                for move in piece.getMoves(white_pieces, black_pieces):
                    new_heuristic = heuristic
                    new_black_pieces = black_pieces
                    if move in black_pieces:
                        new_black_pieces = black_pieces.copy()
                        new_heuristic = heuristic
                        new_black_pieces.pop(move)
                    yield new_black_pieces, Board.makeMove(white_pieces, position, move), new_heuristic
        else:
            for position, piece in black_pieces.items():
                for move in piece.getMoves(white_pieces, black_pieces):
                    new_heuristic = heuristic
                    new_white_pieces = white_pieces
                    if move in white_pieces:
                        new_white_pieces = white_pieces.copy()
                        new_heuristic -= white_pieces[move].heuristic
                        new_white_pieces.pop(move)
                    yield Board.makeMove(black_pieces, position, move), new_white_pieces, new_heuristic
        yield None
    ...
    def getAllInitialValidSuccessors(white_pieces, black_pieces, heuristic):
        for position, piece in white_pieces.items():
            for move in piece.getMoves(white_pieces, black_pieces):
                ...
                if move in black_pieces:
                    new_black_pieces = black_pieces.copy()
                    new_heuristic += black_pieces[move].heuristic
                    new_black_pieces.pop(move)
                yield new_black_pieces, Board.makeMove(white_pieces, position, move), new_heuristic, ((piece.x(), piece.y()),move)
        yield None
...
def ab(node, depth, is_max, alpha, beta):
    ...
    all_valid_successors = Board.getAllValidSuccessors(white_pieces, black_pieces, True, heuristic) if is_max else Board.getAllValidSuccessors(white_pieces, black_pieces, False, heuristic)
